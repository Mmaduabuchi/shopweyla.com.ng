<?php
  ...
  $emailErr = $emailConfirmErr = $successmsg = $errormsg = '';
  ...
  $userkeyid = $_SESSION['userkeyid'];
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($email) || empty($confirmEmail)) {
            //throw error  
            $emailErr = 'Email input is Empty.';
        } else {
            if (ctype_space($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $emailErr = 'Incorrect Email Format.';
            } else {
                //confirm password match
                if ($email !== $confirmEmail) {
                    //throw error  
                    $emailConfirmErr = 'Email did not match.';
                } else {
                    $db = "UPDATE sellerstable SET email = '$email' WHERE userkey = '$userkeyid'";
                    $result = mysqli_query($connection, $db);
                    if (!$result) {
                        $errormsg = 'An error occured while trying to change your Email.';
                    } else {
                        $successmsg = 'Email changed successfully.';
                    }              
                }
            }          
        }      
    ...
    <div class="row mt-5">
        ...
        <div class="col">
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>">
                <label class="fw-bold">Enter Your New Email Address</label>
                <span class="text-success successmsg">
                    <?php echo $successmsg; ?>
                </span>
                <input type="email" name="email" id="encryptedPassword" class="form-control p-3" placeholder="New Email" required>
                <span class="text-danger fw-bold ">
                    <?php echo $emailErr; ?>
                </span>
                <br>
                <input type="email" name="confirmemail" id="encryptedPassword" class="form-control p-3" placeholder="Confirm Email" required>
                <span class="text-danger fw-bold ">
                    <?php echo $emailConfirmErr .' '. $errormsg; ?>
                </span>
                <br>
